...
import { updateDismantlingPotentialRemark } from "./updateDismantlingPotentialRemark"
import ensureUserIsAuthenticated from "../../../../lib/auth/ensureAuthenticated"

jest.mock("../../../../lib/auth/ensureAuthenticated", () => jest.fn())

describe("updateDismantlingPotentialRemark", () => {
  ...
  describe("authorization", () => {
    ...
    it("throws UnauthorizedError if user does not exist", async () => {
      ...
      await expect(updateDismantlingPotentialRemark(product1Id, testRemark)).resolves.toMatchObject({
        success: false,
    ...
    it("throws ZodError if productId is null", async () => {
      ...
      await expect(updateDismantlingPotentialRemark(null, testRemark)).resolves.toMatchObject({
        success: false,
    ...
    it("throws ZodError if productId is undefined", async () => {
      ...
      await expect(updateDismantlingPotentialRemark(undefined, testRemark)).resolves.toMatchObject({
        success: false,
        errorI18nKey: "errors.validation",
      })
    })
    ...
    it("throws ZodError if productId is not a number", async () => {
      ...
      await expect(
        ...
        updateDismantlingPotentialRemark("invalidProductId", testRemark)
    ...
    it("throws UnauthorizedError if user is the project owner but the product is not in that project", async () => {
      ...
      await expect(updateDismantlingPotentialRemark(product2Id, testRemark)).resolves.toMatchObject({
        success: false,
        errorI18nKey: "errors.unauthorized",
      })
    })
    ...
    it("throws UnauthorizedError if user is in a group that lacks project access", async () => {
      ...
      await expect(updateDismantlingPotentialRemark(product3Id, testRemark)).resolves.toMatchObject({
        success: false,
        errorI18nKey: "errors.unauthorized",
      })
    })
