...
import { updateProductTBaustoffAndRemoveDisturbingSubstances } from "lib/domain-logic/circularity/products/manageProductData"

/**
 * Server action for updating a product's TBaustoff reference
 *
 * Handles validation, authentication, and authorization, then delegates to domain logic
 */
export const updateTBaustoffProduct = async (productId: number, selectedId: number) => {
  return withServerActionErrorHandling(async () => {
    ...
    await updateProductTBaustoffAndRemoveDisturbingSubstances(productId, selectedId)
  })
}
