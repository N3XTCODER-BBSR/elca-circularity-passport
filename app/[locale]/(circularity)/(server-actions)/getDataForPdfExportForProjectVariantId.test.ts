...
import getDataForPdfExportForProjectVariantId from "./getDataForPdfExportForProjectVariantId"

jest.mock("../../../../lib/auth/ensureAuthenticated", () => jest.fn())

describe("getDataForPdfExportForProjectVariantId", () => {
  ...
  describe("authorization", () => {
    ...
    it("returns unauthorized errorI18nKey if user does not exist", async () => {
      ...
      await expect(getDataForPdfExportForProjectVariantId(variant1Id, project1Id)).resolves.toMatchObject({
    ...
    // TODO: fix test and refactor domain logic to handle this case
    ...
    //   await expect(
    //     getDataForPdfExportForProjectVariantId(variantIdNotInProject, project1Id)
    ...
    describe("access tokens", () => {
      ...
      it("resolves and returns data if user holds an edit-access token", async () => {
        ...
        const data = await getDataForPdfExportForProjectVariantId(variant1Id, project1Id)
    ...
    it("resolves and returns data if user is the project owner (and variant is part of project)", async () => {
      const mockSession = createMockSession(user1Id)
      ;(ensureUserIsAuthenticated as jest.Mock).mockResolvedValue(mockSession)

      const result = await getDataForPdfExportForProjectVariantId(variant1Id, project1Id)
