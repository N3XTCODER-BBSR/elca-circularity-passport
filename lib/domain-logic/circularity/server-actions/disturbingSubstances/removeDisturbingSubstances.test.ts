...
describe("removeDisturbingSubstanceSelection", () => {
  ...
  const user1Id = 2
  const group1Id = 3
  const variant1Id = 1 // belongs to project1
  ...
  describe("authorization", () => {
    ...
    it("throws UnauthorizedError if user does not exist", async () => {
      ...
      await expect(
        removeDisturbingSubstanceSelection(variant1Id, project1Id, product1Id, disturbingSubstanceSelectionId!)
    ...
    it("throws error with code P2025 if user is project owner but variant is not in the project", async () => {
      // user1 owns project1, but variantIdNotInProject references project2
    ...
    describe("product id validations", () => {
      it("throws ZodError if product id is null or undefined", async () => {
        ...
        await expect(
          ...
          removeDisturbingSubstanceSelection(variant1Id, project1Id, null, disturbingSubstanceSelectionId!)
        ).rejects.toThrow(ZodError)

        // undefined
        await expect(
          // @ts-expect-error Testing undefined
          removeDisturbingSubstanceSelection(variant1Id, project1Id, undefined, disturbingSubstanceSelectionId!)
        ).rejects.toThrow(ZodError)
      })
