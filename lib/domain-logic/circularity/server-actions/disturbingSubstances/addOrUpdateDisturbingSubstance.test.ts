...
describe("addOrUpdateDisturbingSubstanceSelection", () => {
  ...
  describe("authorization", () => {
    ...
    it("throws an error (code: P2025) if user is project owner but variant is not part of project", async () => {
      const mockSession = createMockSession(user1Id)
      ;(ensureUserIsAuthenticated as jest.Mock).mockResolvedValue(mockSession)
    ...
    describe("access tokens", () => {
      ...
      it("throws UnauthorizedError if user only holds a read-only token", async () => {
